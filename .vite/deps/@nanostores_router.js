import { b as atom, h as onMount } from "./nanostores-BeIGPeTF.js";
//#region node_modules/.pnpm/@nanostores+router@1.0.0_nanostores@1.1.0/node_modules/@nanostores/router/index.js
function createRouter(routes, opts = {}) {
	let router = /* @__PURE__ */ atom();
	router.routes = Object.keys(routes).map((name) => {
		let pattern = routes[name];
		if (typeof pattern !== "string") return [name, ...[pattern].flat()];
	...
	let click = (event) => {
		...
		if (link && event.button === 0 && link.target !== "_blank" && link.origin === location.origin && link.rel !== "external" && link.target !== "_self" && !link.download && !event.altKey && !event.metaKey && !event.ctrlKey && !event.shiftKey && !event.defaultPrevented) {
			...
			if (hashChanged) {
				...
				if (link.hash === "" || link.hash === "#") window.dispatchEvent(new HashChangeEvent("hashchange"));
			}
		}
	};
	let set = router.set;
	...
	if (typeof window !== "undefined" && typeof location !== "undefined") onMount(router, () => {
		let page = parse(location.href);
		if (page !== false) set(page);
	...
	router.open = (path, redirect) => {
		let page = parse(path);
		if (page !== false) {
			if (typeof history !== "undefined") if (redirect) history.replaceState(null, null, path);
...
//#endregion
export { createRouter, getPagePath, openPage, redirectPage };
